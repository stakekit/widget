...
import { useWalletController, walletStateAtom } from "../runtime/root-atom";

export const useSKWallet = () => {
  const controller = useWalletController();
  const walletStateResult = useAtomValue(
    walletStateAtom(controller.initializationKey)
  );
  const initialWalletState: NormalizedWalletState = controller.isLoading
    ? {
