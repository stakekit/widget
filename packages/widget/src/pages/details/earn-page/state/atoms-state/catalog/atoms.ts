        ...
        Effect.gen(function* () {
          ...
          const availability = yield* Effect.all(
            key.categoryOrder.map((category) =>
              Effect.gen(function* () {
                const page = yield* api.yield.getAvailableYields({
  ...
  widgetAtomRuntime
    .atom(() =>
      Effect.gen(function* () {
        ...
        return yield* api.yield.getInitialYield(key.yieldId);
  ...
  widgetAtomRuntime
    .atom(() =>
      Effect.gen(function* () {
        ...
        const api = yield* StakeKitApiService;
        const positions = yield* api.yield.getCatalogPositions({
          address: key.address,
          network: key.network,
        });
      ...
      fetchPage: (offset) =>
        Effect.gen(function* () {
          ...
          if (
            ...
            const page = yield* api.yield.getYieldTokens({
              limit: DEFAULT_PAGE_SIZE,
              offset,
          ...
          const tokens = yield* api.legacy.getLegacyTokenOptions(
  ...
  widgetAtomRuntime
    .atom(() =>
      Effect.gen(function* () {
        ...
        const api = yield* StakeKitApiService;
        const balances = yield* api.legacy.scanEarnTokenBalances({
          address: key.address,
          ...
          (context) => {
            return paginatedApiStream({
              fetchPage: (offset) =>
                Effect.gen(function* () {
                  const api = yield* StakeKitApiService;
                  const page = yield* api.yield.getValidators({
                    limit: DEFAULT_PAGE_SIZE,
