...
export const getProviderDetails = ({
  ...
  validatorsData?: ReadonlyArray<ValidatorDto>;
}): Res => {
...
export const useProvidersDetails = ({
  ...
  integrationData: Maybe<Yield>;
  validatorsAddresses: Maybe<ReadonlyArray<string> | Map<string, ValidatorDto>>;
  selectedProviderYieldId: Maybe<string>;
  validatorsData?: Maybe<ReadonlyArray<ValidatorDto>>;
}) => {
  ...
  const shouldFetchValidators = validatorsAddresses
    .filter((val): val is ReadonlyArray<string> => !(val instanceof Map))
