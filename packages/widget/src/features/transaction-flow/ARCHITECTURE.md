...
Three unchanged legacy views are temporarily excluded from one applicable syntax rule and protected by exact content hashes: two Transaction Workflow lifecycle adapters and the mixed React/Effect gas-warning adapter. Touching one invalidates its baseline: remove the legacy application behavior and delete both its rule exclusion and hash entry rather than refreshing the hash.
