...
One unchanged legacy Transaction Workflow presentation adapter is temporarily excluded from one applicable syntax rule and protected by an exact content hash. Touching it invalidates its baseline: remove the legacy application behavior and delete both its rule exclusion and hash entry rather than refreshing the hash.
