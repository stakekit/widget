...
describe("classic completion urls", () => {
  it("includes confirmation explorerUrl in completion urls", async () => {
    const result = await runClassicToCompletion("tx-with-url", {
      getTransactionStatus: () =>
        Effect.succeed({
  ...
  it("keeps submit explorerUrl when status omits it", async () => {
    const submit = vi.fn(() =>
      Effect.succeed({
    ...
    const result = await runClassicToCompletion("tx-submit-url-only", {
      getTransactionStatus: () =>
        Effect.succeed({
          explorerUrl: null,
          status: "CONFIRMED",
