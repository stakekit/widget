...
const captureDiagnostics = <A, E>(effect: Effect.Effect<A, E>) => {
  ...
  return {
    ...
    result: Effect.runPromise(
      effect.pipe(Effect.provide(Logger.layer([logger])))
    ),
  };
};

describe("API response schemas", () => {
  it("strictly decodes a valid single domain model", async () => {
    const result = await Effect.runPromise(
      Schema.decodeUnknownEffect(Item)(validItem("item-1"))
    );
  ...
  it("rejects a malformed single domain model", async () => {
    await expect(
      Effect.runPromise(
  ...
  it("keeps valid items in source order and rejects complete invalid parents", async () => {
    ...
    const result = await decoded.result;
  ...
  it("returns an empty collection when every top-level item is rejected", async () => {
    const result = await Effect.runPromise(
      Schema.decodeUnknownEffect(Items)([
  ...
  it("omits an entire key-value entry when its key or value fails", async () => {
