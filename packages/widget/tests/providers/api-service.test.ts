...
describe("application API services", () => {
  it.effect(
    "constructs separated Legacy, Yield, and Borrow operation surfaces",
    () =>
      Effect.gen(function* () {
        const context = yield* Layer.build(
          makeTestStakeKitApiLayer(config)
        ).pipe(Effect.scoped);
        ...
        const walletBootstrapSource = Context.get(
          context,
  ...
  it.effect(
    "fails only Borrow capabilities when Borrow configuration is missing",
    () =>
      Effect.gen(function* () {
        const context = yield* Layer.build(
        ...
        expect(
          yield* Effect.flip(operations.getAction("borrow-action"))
        ).toBeInstanceOf(MissingBorrowApiConfig);
      })
  );

  it.effect(
    "maps Borrow market reads through the narrow source capability",
    () =>
      Effect.gen(function* () {
        const markets = vi.fn(() =>
        ...
        expect(yield* source.getMarkets(request)).toEqual({
          items: [],
          limit: 100,
          offset: 0,
          total: 0,
  ...
  it.effect(
    "maps Borrow action polling through the narrow operation capability",
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* operations.getAction("borrow-action")).toBe(null);
        expect(getAction).toHaveBeenCalledWith("borrow-action", undefined);
      })
  );

  it.effect("fails disabled Borrow commands before calling transport", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* Effect.flip(operations.getAction("borrow-action"))
      ).toBeInstanceOf(BorrowFeatureDisabled);
      expect(getAction).not.toHaveBeenCalled();
    })
  );

  it.effect(
    "returns decoded domain values from successful transport responses",
    () =>
      Effect.gen(function* () {
        const source = makeYieldResourceSource({
          KycControllerGetStatus: () =>
            Effect.succeed({ kycStatus: "approved" }),
        } as never);

        const result = yield* source.getKycStatus({
          address,
  ...
  it.effect("maps Health through the Yield read capability", () =>
    Effect.gen(function* () {
      const health = vi.fn(() =>
        Effect.succeed({
          status: "OK",
  ...
  it.effect(
    "maps Enabled Wallet Networks through the Yield read capability",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "maps Action Preview through the narrow Yield operation capability",
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* operations.previewAction({ command, intent: "enter" })
        ).toEqual(expected);
  ...
  it.effect(
    "maps Yield position requests through the narrow read capability",
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* source.getPositions(command);
  ...
  it.effect(
    "maps Yield directory filters through the narrow read capability",
    () =>
      Effect.gen(function* () {
        const list = vi.fn(() =>
          Effect.succeed({ items: [], limit: 100, offset: 0, total: 0 })
        ...
        expect(yield* source.listYields(request)).toEqual({
          items: [],
          limit: 100,
          offset: 0,
          total: 0,
  ...
  it.effect(
    "maps the canonical Earn Catalog through the Legacy read capability",
    () =>
      Effect.gen(function* () {
        ...
        yield* legacySource.getTokenOptions({
          enter: true,
          network: "ethereum",
          yieldTypes: ["staking"],
        });
  ...
  it.effect(
    "maps complete validator query identity through the Yield read capability",
    () =>
      Effect.gen(function* () {
        ...
        yield* source.listValidators(request);
  ...
  it.effect(
    "maps Activity history query identity through the Yield read capability",
    () =>
      Effect.gen(function* () {
        ...
        yield* source.listActivity(request);
  ...
  it.effect(
    "maps an Activity action through the owner-scoped read capability",
    () =>
      Effect.gen(function* () {
        const action = yieldApiActionDtoFixture();
        const expected = Schema.decodeUnknownSync(YieldAction)(action);
        ...
        const result = yield* source.getActivityAction(expected.id);
  ...
  it.effect(
    "maps single-Yield and gas balances through narrow read capabilities",
    () =>
      Effect.gen(function* () {
        ...
        yield* yieldSource.getSingleYieldBalances({
          address,
          yieldId: firstYieldId,
        });
        yield* legacySource.getGasTokenBalances(gasCommand);
  ...
  it.effect("maps price requests through the Legacy read capability", () =>
    Effect.gen(function* () {
      ...
      yield* source.getPrices(request);
  ...
  it.effect(
    "maps Yield history identity through the Yield read capability",
    () =>
      Effect.gen(function* () {
        ...
        yield* source.getRewardRateHistory(request).pipe(Effect.flip);
        yield* source.getTvlHistory(request).pipe(Effect.flip);
  ...
  it.effect(
    "maps opportunity and provider lookups through the narrow read capability",
    () =>
      Effect.gen(function* () {
        ...
        expect(yield* source.getOpportunity(yieldModel.id)).toEqual(yieldModel);
        expect(yield* source.getProvider(provider.id)).toEqual(
  ...
  it.effect(
    "reports invalid consumed opportunity arguments as response decode errors",
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* Effect.flip(source.getOpportunity(firstYieldId))
        ).toBeInstanceOf(ResponseDecodeError);
      })
  );

  it.effect(
    "maps a provider 404 to explicit absence at the source boundary",
    () =>
      Effect.gen(function* () {
        ...
        expect(Option.isNone(yield* source.getProvider(provider.id))).toBe(
          true
        );
      })
  );
  ...
  it.effect(
    "maps transport failures independently from response decoding failures",
    () =>
      Effect.gen(function* () {
        ...
        const requestError = yield* transportFailureSource
          .getKycStatus({ address, yieldId: firstYieldId })
          .pipe(Effect.flip);
        const decodeError = yield* malformedResponseSource
          .getKycStatus({ address, yieldId: firstYieldId })
          .pipe(Effect.flip);
  ...
  it.effect(
    "retains valid aggregate entries when a sibling response is malformed",
    () =>
      Effect.gen(function* () {
        const rewards = {
          last24H: "0",
        ...
        const result = yield* source.getRewardsSummaries({
          addresses,
