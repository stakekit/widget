...
describe("Borrow Flow Atom bridge", () => {
  it("projects service state and binds Session acquisition to the root Atom lifetime", async () => {
    ...
    if (!rootAtom) throw new Error("Expected a Borrow Flow Session root Atom");
  ...
  it("binds Review and Execution handles to route scope and forwards their commands", async () => {
    const session = makeSession(1);
    const current = await Effect.runPromise(
    ...
    const service = BorrowTransactionFlowService.of({
      acquireSession: () =>
        Effect.acquireRelease(
          Effect.succeed({ _tag: "Acquired", session: sessionHandle } as const),
          () => Effect.void
        ),
      currentSession: SubscriptionRef.changes(current),
    ...
    releaseReview();
    await vi.waitFor(() => expect(reviewProbes.released).toBe(1));
    ...
    registry.set(executionModule.facade.workflowCommandAtom, { _tag: "Retry" });
    ...
    releaseExecution();
    await vi.waitFor(() => expect(executionProbes.released).toBe(1));
