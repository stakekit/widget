...
React obtains the runtime-owned router synchronously through an internal Atom and passes it to `RouterProvider`; feature modules do not receive the raw router. Closing the Application Runtime Generation by unmounting the Widget disposes the router, and a later mount starts a fresh router with fresh memory history. Per ADR-0016, generation identity consists of normalized API configuration plus mount-time feature configuration such as `borrowEnabled`; changing it while mounted is an invariant violation. Live settings changes within the same generation preserve the router.
